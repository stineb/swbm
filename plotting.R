...
{modified=T}else{modified=F}




######### plot functions of normalized ET and normalized runoff versus soil moisture ########################
if(!is.element(2,dev.list())){x11(width=7,height=4.5,ypos=700)}else{dev.set(2)}
plot.new()
...
plot(help1,type='l',col=2,ylim=c(-0.04,1),lwd=2,
ylab='Q/P and ET/Rnet',xlab='Soil Moisture (mm)')
lines(help2,lwd=2)
abline(h=0)

if (modified){
legend('topleft', c("Q/P default","Q/P current",'ET/Rnet default','ET/Rnet current'),col = c(2,2,1,1),
text.col = 'black', lwd=c(1.5,1.5,1.5,1.5),lty=c(1,2,1,2),cex=0.85)
 
legend(80,1.04, c("Soil Moisture Histogram Default","Soil Moisture Histogram Current"),
col = c('grey85',1),text.col = 'black', lwd=c(10,1.5),lty=c(1,2),cex=0.85)
}else{
legend('topleft', c("Q/P",'ET/Rnet'),col = c(2,1),
text.col = 'black', lwd=c(1.5,1.5),cex=0.85)
legend(80,1.04, c("Soil Moisture Histogram Default"),
col = c('grey85'),text.col = 'black', lwd=c(10),lty=c(1),cex=0.85)
}
...
lines(c(0+(i-1)*20,20+(i-1)*20),
rep(counts,2),lty=2)
lines(c(20+(i-1)*20,20+(i-1)*20),
c(counts,counts_next),
lty=2)
}
...
axis(1,at=c(seq(25,175,50),210),labels=c(round(sm_et_coupling,digits=2),'cor (SM,ET)'),
lwd=2,tick=F,col.axis=4)
par(mgp=c(1.5,0.3,0))
#############################################################################################################
...
if(!is.element(3,dev.list())){x11(width=9,height=4,xpos=1000,ypos=0)}else{dev.set(3)}
plot.new()
...
soilm_obs[co]=(7.5*mean(s[i:(i+23),7])+12.5*mean(s[i:(i+23),8])+
              20*mean(s[i:(i+23),9])+25*mean(s[i:(i+23),10])+
              30*mean(s[i:(i+23),11]))/95
co=co+1
}
